...
export async function POST(request: Request) {
  try {
    ...
    const requestedFamilyId = String(body.familyId ?? "");
    ...
    const [existingCount, family] = await Promise.all([
      prisma.account.count(),
      requestedFamilyId
        ? prisma.family.findUnique({ where: { id: requestedFamilyId } })
        : prisma.family.findFirst({ orderBy: { createdAt: "asc" } })
    ]);
    const account = await prisma.account.create({
      data: {
        ...(family ? { family: { connect: { id: family.id } } } : {}),
        name,
