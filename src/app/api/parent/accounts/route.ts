...
export async function POST(request: Request) {
  try {
    ...
    const existingCount = await prisma.account.count({ where: { familyId: parent.familyId } });
    const account = await prisma.account.create({
      data: {
        family: { connect: { id: parent.familyId } },
        name,
        avatarUrl: `https://api.dicebear.com/9.x/adventurer/svg?seed=${encodeURIComponent(name)}&backgroundColor=b6e3f4,c0aede,d1d4f9`,
        currentBalance: 0,
